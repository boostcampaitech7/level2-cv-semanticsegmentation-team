...
class EarlyStopping:
    def __init__(self, patience=7, min_delta=1e-4, verbose=True, mode='max'):
        """
        Args:
            patience (int): 성능 개선이 없을 때 기다릴 에폭 수
            min_delta (float): 성능 개선으로 간주할 최소 변화량
            verbose (bool): 자세한 출력 여부
            mode (str): 'min' 또는 'max'. 
                       - 'min': 값이 감소하는 것이 좋음 (loss)
                       - 'max': 값이 증가하는 것이 좋음 (accuracy, dice)
        """
...
class Trainer:
    ...
    def train_fold(self, fold_num: int, fold_data: dict):
        ...
        for epoch in range(self.num_epochs):
            ...
            early_stopping(val_dice)  # loss 기준으로 early stopping

...
class FullDataset(Dataset):
    def __init__(self, image_root, gt_root, size, mode):
        ...
        # 이미지와 마스크 매칭 확인
        self._check_image_mask_matching()
    ...
    def _check_image_mask_matching(self):
        """Check if images and masks are properly matched."""
        mismatched_files = []
        for img_path, mask_path in zip(self.images, self.gts):
            img_name = os.path.splitext(os.path.basename(img_path))[0]
            mask_name = os.path.splitext(os.path.basename(mask_path))[0]
            if img_name != mask_name:
                mismatched_files.append((img_name, mask_name))

        if mismatched_files:
            print("Error: Image and mask files are not properly matched.")
            for img, mask in mismatched_files:
                print(f"Mismatched: Image: {img}, Mask: {mask}")
            raise ValueError("Image and mask file mismatch detected. Please check the dataset.")

        print("All image and mask files are properly matched.")

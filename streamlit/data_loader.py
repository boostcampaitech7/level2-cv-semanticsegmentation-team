...
#     return annotations
def extract_annotations_from_csv(df, image_name, height, width):
    """DataFrame에서 특정 이미지에 대한 어노테이션을 추출하고 폴리곤 좌표로 변환."""
    if df is None:
        st.error("CSV DataFrame is None")
        return []
